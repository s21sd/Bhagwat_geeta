...
const VerseRead = ({ route }) => {
    ...
    // console.log(data.commentaries.length);
            ...
            <ScrollView showsVerticalScrollIndicator={false} nestedScrollEnabled={true}>
                <View className="flex flex-row justify-between items-center mt-3" style={{ flex: 1 }}>
                    <TouchableOpacity
                        ...
                        onPress={() => navigation.navigate('Mainscreen')} className=" bg-white mx-4 flex justify-center items-center rounded-full">
                ...
                <View className="flex flex-col justify-center items-center px-5">
                    <Text style={{ fontSize: hp(3) }} className="font-bold ">{data.chapter_number}.{data.verse_number}</Text>
                    <Text style={{ fontSize: hp(2.5) }} className="font-semibold text-[#FF8600] mt-3 text-center">{data.text}</Text>
                </View>
                ...
                <View className="flex flex-col justify-center items-center px-5">
                    ...
                    {data.translations && data.translations.map((translation, index) => (
                        <View key={index} className="mb-2">
                            <Text style={{ fontSize: hp(2) }} className="font-bold mt-6">{translation.description}</Text>
                            <Text style={{ fontSize: hp(2) }} className="font-samibold text-right mt-3">Author Name: {translation.author_name}</Text>
                        </View>
                    ))}
                    <View className="w-[100%] border border-gray-400 mt-2" />
                    <Text style={{ fontSize: hp(3) }} className="font-bold mt-4">COMMENTARIES</Text>
                    {data.commentaries && data.commentaries.map((commentary, index) => (
                        <View key={index} className=" mt-6">
                            <ReadMore style={{ fontSize: hp(2), fontWeight: 'bold' }} numberOfLines={5}>
                                <Text className="font-bold mt-6" style={{ fontSize: hp(2), marginTop: 10 }}>{commentary.description}</Text>
                            </ReadMore>
                            <Text style={{ fontSize: hp(2) }} className="font-samibold text-right mt-3 mb-3">Author Name: {commentary.author_name}</Text>
                        </View>
                    ))}
                </View>
            </ScrollView>
